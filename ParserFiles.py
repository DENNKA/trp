...
class ParserFiles():
    ...
    def files_to_episodes(self, files, root):
        """
        ...
        for path in paths:
            ...
            if file_type != "fonts":
                ...
                if file_type == "subtitle":
                    try:
                        files_dict[file_episode][file_type][str(Path(*path.parts[1 + subtitle_in_folder:-1]))] = str(root / path)
                    except TypeError:
                        files_dict[file_episode][file_type] = defaultdict()
                        files_dict[file_episode][file_type][str(Path(*path.parts[1 + subtitle_in_folder:-1]))] = str(root / path)
                        pass
                else:
                    files_dict[file_episode][file_type] = str(root / path)
            else:
